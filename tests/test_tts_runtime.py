...
def test_live_playback_resume(monkeypatch, tmp_path: Path) -> None:
    src = tmp_path / "chapter.txt"
    src.write_text("A\n\nB\n\nC", encoding="utf-8")
    target = TTSTarget(source=src, output=tmp_path / "chapter.mp3")
    cache_root = tmp_path / "cache"
    ...
    monkeypatch.setattr(
        ...
        lambda wav_paths, output_path, **kwargs: output_path.write_bytes(b"".join(path.read_bytes() for path in wav_paths)),
    )

    playback_calls: list[int] = []

    def playback_with_failure(path: Path) -> None:
        idx = int(path.stem.split("_")[0])
        playback_calls.append(idx)
        if idx == 2:
            raise RuntimeError("stop after second chunk")
    ...
    with pytest.raises(RuntimeError):
        synthesize_texts_to_mp3(
            ...
            keep_cache=True,
            live_playback=True,
            playback_callback=playback_with_failure,
        )

    cache_dir = _target_cache_dir(cache_root, target)
    progress_file = cache_dir / ".progress"
    assert progress_file.exists()
    assert progress_file.read_text(encoding="utf-8").strip() == "1"
    assert playback_calls == [1, 2]

    resumed_calls: list[int] = []

    def playback_resume(path: Path) -> None:
        resumed_calls.append(int(path.stem.split("_")[0]))
    ...
    synthesize_texts_to_mp3(
        ...
        keep_cache=True,
        live_playback=True,
        playback_callback=playback_resume,
    )
